...
> **存档时间**: 2026-06-11 09:10 UTC
> **线上复核**: 2026-06-15（确认精准 API、URL batch、`page_ranges`、限流策略、200 页限制与 `full_zip_url` 字段仍可见）
...
| 单文件页数 | 150 份中 125 份超过 200 页，最大 374 页 | ⚠️ 使用 `page_ranges` 拆为 `1-200` / `201-end` |
| 输出需求 | Markdown 正文 + 表格数据 | 精准解析 API（首版用 `pipeline`，保留 `vlm` 作为质量增强选项） |
| 网络环境 | 国内 | ✅ 国内 URL 优先，避免 GitHub/AWS 等国外源 |

> 💡 **当前实现**: 项目保留 **本地 MinerU CLI 模式** 作为默认 backend；同时新增显式 API backend，用 `page_ranges` 分段提交精准 API，最终仍输出 `data/parsed/{doc_id}.md`。详见 [附录 B](#附录-b与项目代码的对应关系)。
...
### B.2 云端 API 模式（新增 smoke backend）

项目新增 [`src/parse/mineru_api_parser.py`](src/parse/mineru_api_parser.py)，通过显式参数启用：

```powershell
uv run python src/main.py --stage parse --parse-backend api --limit 1
```

运行前需在环境变量或 `.env` 中配置 `MINERU_API_KEY`。不要把 token 写入 git。

API backend 流程：

1. 读取 `metadata.csv` 中的 `pdf_url`，不上传本地 PDF。
2. 用 PyPDF2 读取本地 PDF 页数，按 200 页拆分 `page_ranges`。
3. `POST /api/v4/extract/task` 创建每个分段任务。
4. 轮询 `GET /api/v4/extract/task/{task_id}`，任务完成后下载 `full_zip_url`。
5. 解压结果包，提取 `full.md`，按页段顺序合并为 `data/parsed/{doc_id}.md`。

中间产物：

- `data/parsed/mineru_api_tasks.jsonl`：API 分段任务记录。
- `data/parsed/mineru_api_raw/{doc_id}/part_*/`：每段原始 zip 与解压结果。

当前实现边界：

- `--parse-backend api` 是单任务 smoke backend。它按 PDF 和 page segment 顺序提交任务，适合验证 token、URL 可访问性、分段、断点和输出结构。
- `--parse-backend api-batch` 是 URL batch backend。它把每个 `doc_id + page_range` 作为一个批量 `files[]` 项提交到 `/api/v4/extract/task/batch`，再用 `/api/v4/extract-results/batch/{batch_id}` 查询。
- 已有断点能力：`success` 且本地 raw `full.md` 非空的 segment 会复用；已有 `result.zip` 会补解压；`ready/download_failed` 会补下载；`submitted` 但未完成的 segment 会继续轮询原 task_id。
- 文件管理约定：最终 Markdown 写 `data/parsed/{doc_id}.md`；原始 API zip 和解压结果写 `data/parsed/mineru_api_raw/{doc_id}/part_XX/`；任务断点和审计日志写 `data/parsed/mineru_api_tasks.jsonl`。
- 2026-06-15 小样本验证：`api-batch --limit 2` 成功解析 2 份年报；二次运行复用缓存，不重复提交 batch。随后 `api-batch --limit 5` 成功解析 5 份年报，`api-batch --limit 20` 成功生成 20 份非空 Markdown。
- 2026-06-15 全量验证：`api-batch` 完成 150 份年报全量解析，最终 `data/parsed/*.md` 为 150，`mineru_api_tasks.jsonl` 最新状态为 275 个 page segment 全部 `success`，失败 0。
- 网络恢复口径：结果 zip 下载阶段可能受 VPN 或 CDN TLS 短断影响。项目已为 `full_zip_url` 下载增加应用层重试；如 segment 已在 batch 中完成但下载失败，后续运行会优先复用本地 `result.zip`，必要时用原 `batch_id/data_id` 刷新 `full_zip_url` 并补下载，不重新提交解析任务。
...
**注意事项**：
   - 单任务仍有 200 页限制，长年报需要用 `page_ranges` 拆段
   - URL batch 每次提交也应控制批量大小，避免触发队列满或任务数量上限；本项目默认每批 50 个 page segment，并发下载 8 个结果包、并发解压 4 个结果包；本机可通过 `.env` 临时调高 worker 加速
   - 提交任务接口共享 50 files/min 限流；单用户每天最多 5000 个文件，html 文件最多 100 个
   - 获取任务结果接口共享 1000 次/min 限流；本项目 5 秒轮询远低于该上限
   - 每日 1000 页最高优先级额度，超出后排队
   - 国内 URL 优先，国外 URL（GitHub、AWS 等）可能超时
   - 跑 API parse 时建议关闭 VPN 或确认 `cdn-mineru.openxlab.org.cn` 连接稳定；如果只是下载结果包短断，可直接重跑同一命令续跑
   - 建议设置合理的轮询间隔（3–5 秒）
...
| 场景 | 推荐模式 | 原因 |
|------|---------|------|
| 单份短文档 / 无 API token | 本地 CLI | 无需网络与额度 |
| 无 GPU / 远程服务器 | 云端 API | 无需配置 CUDA 环境 |
| 本项目 150 份长年报 | `api-batch` | 可分段、可断点、可批量提交，避免本地 GPU 长时间占用 |
| 需要最高精度 | API `vlm` 或本地高精度后端 | 首版 smoke 先用 `pipeline` 控制时间与成本 |
