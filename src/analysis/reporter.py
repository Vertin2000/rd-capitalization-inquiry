"""Generate a classroom-ready Markdown report from pipeline outputs."""
...
def _baseline_row(label: str, data: Any) -> str:
    """Render one baseline comparison row; returns empty string if data missing."""
    if not isinstance(data, dict):
        return f"| {label} | — | — | — | — | — | — |"
    return (
        f"| {label} | {_fmt(data.get('predicted_positive'))} | "
        f"{_fmt(data.get('tp'))} | {_fmt(data.get('fp'))} | {_fmt(data.get('fn'))} | "
        f"{_fmt(data.get('precision'))} | {_fmt(data.get('recall'))} |"
    )
...
def _group_average(
    ...
    for row in rows:
        group = str(row.get(group_key) or "未知")
        grouped.setdefault(group, []).append(row)
    return {
        group: _avg(group_rows, value_key)
        for group, group_rows in sorted(grouped.items())
    }


class FinalReporter:
    """Write a presentation-oriented final report."""

    def __init__(
        self,
        evaluation_path: str | Path = "outputs/loop_evaluation.json",
        ...
        self.inquiry_path = (
            self._resolve(inquiry_path)
            if inquiry_path is not None
            else self.scored_path.parent.parent / "inquiry" / "inquiry_records.jsonl"
        )
    ...
    def run(self) -> dict[str, int]:
        evaluation = _read_json(self.evaluation_path)
        ...
        industry_weight = float(scoring_config.get("industry_weight", 0.25))
        change_weight = float(scoring_config.get("change_weight", 0.25))
        fuzziness_weight = float(scoring_config.get("fuzziness_weight", 0.25))
        anomaly_percentile = float(scoring_config.get("anomaly_percentile", 0.80))
        identity_multiplier = float(
            scoring_config.get("identity_multiplier") or scoring_config.get("identity_weight", 0.25)
        )
        matrix = evaluation.get("confusion_matrix", {})
        metrics = evaluation.get("metrics", {})
        top_k = evaluation.get("top_k") or []
        baselines = evaluation.get("baselines") or {}
        tn_water = evaluation.get("tn_water") or {}
        caveats = evaluation.get("caveats") or [
            "Partial Run Caveat: downstream inputs are incomplete."
        ]
        anomalies = [row for row in scored_records if row.get("is_anomaly")]
        anomalies.sort(
            key=lambda row: row.get("aggressiveness_score") or -1,
            reverse=True,
        )
        evaluated_count = evaluation.get("total", len(scored_records))
        inquiry_count = evaluation.get("inquiry_records")
        unscored_count = sum(
            1 for row in scored_records if row.get("aggressiveness_score") is None
        )
        valid_scores = [
            row.get("aggressiveness_score")
            for row in scored_records
            if row.get("aggressiveness_score") is not None
        ]
        average_score = (
        ...
        valid_rate_rows = [
            row
            for row in scored_records
            if isinstance(row.get("capitalization_rate"), (int, float))
        ]
        valid_score_rows = [
            row
            for row in scored_records
            if isinstance(row.get("aggressiveness_score"), (int, float))
        ]
        scored_count = len(valid_score_rows)
        non_anomalies = [row for row in scored_records if not row.get("is_anomaly")]
        anomaly_rate_avg = _avg(anomalies, "capitalization_rate")
        non_anomaly_rate_avg = _avg(non_anomalies, "capitalization_rate")
        anomaly_score_avg = _avg(anomalies, "aggressiveness_score")
        non_anomaly_score_avg = _avg(non_anomalies, "aggressiveness_score")
        cap_rate_avg = _avg(valid_rate_rows, "capitalization_rate")
        cap_rate_median = _median([row.get("capitalization_rate") for row in valid_rate_rows])
        industry_cap_rate = _group_average(valid_rate_rows, "industry", "capitalization_rate")
        year_cap_rate = _group_average(valid_rate_rows, "year", "capitalization_rate")
        inquiry_with_candidates = sum(
            1 for row in inquiry_records if (row.get("candidate_count") or 0) > 0
        )
        related_inquiries = [
            row for row in inquiry_records if row.get("capitalization_related") or row.get("inquiry_actually_received")
        ]
        evaluated_related = (matrix.get("tp") or 0) + (matrix.get("fn") or 0)
        extraction_coverage = _ratio(evaluated_count, inquiry_count)
        valid_rate_coverage = _ratio(len(valid_rate_rows), evaluated_count)
        anomaly_share = _ratio(len(anomalies), evaluated_count)
        candidate_share = _ratio(inquiry_with_candidates, inquiry_count)
        related_share = _ratio(len(related_inquiries), inquiry_count)
        industry_text = "；".join(
            f"{industry} {_fmt(value)}%"
            for industry, value in industry_cap_rate.items()
        ) or "暂无可用行业结果"
        year_text = "；".join(
            f"{year} 年 {_fmt(value)}%"
            for year, value in year_cap_rate.items()
        ) or "暂无可用年度结果"

        lines = [
            "# 研发资本化风险排序与问询函可行性测试报告",
            "",
            f"> 课堂展示版 · 生成时间：{datetime.now().isoformat(timespec='seconds')}",
            "",
            "## 一句话结论",
            "",
            ...
            "",
            f"基于当前全量样本快照，系统评估了 `{_fmt(evaluated_count)}` 条年报记录，覆盖 `{_fmt(inquiry_count)}` 条问询标签中的 `{_fmt_pct(extraction_coverage)}`。可计算资本化率的记录有 `{len(valid_rate_rows)}` 条，平均资本化率为 `{_fmt(cap_rate_avg)}%`，中位数只有 `{_fmt(cap_rate_median)}%`。这说明少数高资本化样本拉高了平均值；如果只看平均数，会误以为样本整体都比较激进。",
            "",
            f"风险排序标出了 `{len(anomalies)}` 条高风险 company-year，占全样本 `{_fmt_pct(anomaly_share)}`。这个数量是从 `{_fmt(scored_count)}` 条可评分记录里取前 20% 左右得到的，不是直接对 150 条全样本取分位数。异常组平均资本化率为 `{_fmt(anomaly_rate_avg)}%`，非异常组为 `{_fmt(non_anomaly_rate_avg)}%`；异常组平均风险分为 `{_fmt(anomaly_score_avg)}`，非异常组为 `{_fmt(non_anomaly_score_avg)}`。这说明评分器整体能把更值得复核的样本排到前面，但异常名单是行业偏离、跨期变化和披露模糊度的混合信号。",
            "",
            f"问询函这边要说得保守一点：`{_fmt(inquiry_count)}` 个年报样本中有 `{inquiry_with_candidates}` 个样本发现问询候选，`{len(related_inquiries)}` 个样本命中研发资本化相关关键词；在已评估的 `{_fmt(evaluated_count)}` 条记录中，模型命中 `{_fmt(matrix.get('tp'))}` 条真实相关问询，漏掉 `{_fmt(matrix.get('fn'))}` 条，precision 为 `{_fmt(metrics.get('precision'))}`，recall 为 `{_fmt(metrics.get('recall'))}`。也就是说，闭环已经接上了，但预测效果还弱。",
            "",
            "## 项目到底在做什么",
            "",
            ...
            "### 4. 得到了什么结果",
            "",
            f"当前快照已经评估 `{_fmt(evaluated_count)}` 条年报记录，其中 `{len(valid_rate_rows)}` 条可以计算资本化率，`{len(valid_score_rows)}` 条可以得到风险分。资本化率平均值为 `{_fmt(cap_rate_avg)}%`，中位数为 `{_fmt(cap_rate_median)}%`，说明样本不是整体高资本化，而是少数高值样本拉高均值。风险排序最终标出 `{len(anomalies)}` 条高风险样本，占已评估样本 `{_fmt_pct(anomaly_share)}`。",
            "",
            f"异常组和非异常组的差异比较明显：异常组平均资本化率为 `{_fmt(anomaly_rate_avg)}%`，非异常组为 `{_fmt(non_anomaly_rate_avg)}%`；异常组平均风险分为 `{_fmt(anomaly_score_avg)}`，非异常组为 `{_fmt(non_anomaly_score_avg)}`。这说明评分规则整体上把更值得复核的样本推到了前面。但风险分是混合信号，个别资本化率为 0% 的样本也可能因为披露模糊度或跨期变化进入异常名单，不能直接说成激进资本化。",
            "",
            f"问询函这边更需要谨慎。150 个年报样本中，当前发现 `{inquiry_with_candidates}` 个有问询候选，`{len(related_inquiries)}` 个命中研发资本化相关关键词。与风险排序名单合并后，得到 TP=`{_fmt(matrix.get('tp'))}`、FP=`{_fmt(matrix.get('fp'))}`、TN=`{_fmt(matrix.get('tn'))}`、FN=`{_fmt(matrix.get('fn'))}`，precision=`{_fmt(metrics.get('precision'))}`，recall=`{_fmt(metrics.get('recall'))}`。这说明现在的成果是闭环能跑、风险名单能解释；预测监管问询这件事，还没到可以下结论的时候。",
            "",
            "### 5. 这些结果说明什么",
            "",
            f"这些数字给我的判断是：第一，研发资本化风险更像尾部风险，不是全样本平均风险。第二，行业差异不能忽略，医药、软件和电子设备不能硬套同一个阈值。第三，异常名单有实际用处，它把人工复核范围从 `{_fmt(evaluated_count)}` 个 company-year 缩到 `{len(anomalies)}` 个重点样本。第四，问询函验证暂时没有给出强预测效果，可能是部分财务字段仍为空，也可能是关键词标签太粗，还可能是交易所问询本来就不只问研发资本化。",
            "",
            ...
            "### 结论一：研发资本化风险具有明显尾部特征",
            "",
            f"当前可计算资本化率样本的平均值为 `{_fmt(cap_rate_avg)}%`，中位数只有 `{_fmt(cap_rate_median)}%`。这说明多数 company-year 的研发资本化率较低或接近零，少数高值样本把平均数拉了上去。课堂上可以直接讲：这个项目看到的不是普遍异常，而是尾部样本特别值得看。",
            "",
            "### 结论二：行业背景会显著影响资本化率解释",
            "",
            f"当前样本按行业计算的平均资本化率为：{industry_text}。软件信息和医药制造样本高于电子设备样本。这个差异很好理解：不同产业的研发周期、成果形态、无形资产确认方式都不一样。所以评分不能只看绝对资本化率；同样是 20%，放在不同行业里含义可能不同。这里的行业归属来自 `configs/crawl.yaml` 显式 `industry` 字段（20 医药制造 / 20 电子设备 / 10 软件信息），后续如调整股票池需同步维护该字段。",
            "",
            "### 结论三：异常名单抓住了更激进的会计处理样本",
            "",
            f"异常组平均资本化率为 `{_fmt(anomaly_rate_avg)}%`，非异常组为 `{_fmt(non_anomaly_rate_avg)}%`；异常组平均风险分为 `{_fmt(anomaly_score_avg)}`，非异常组为 `{_fmt(non_anomaly_score_avg)}`。这个差距说明，风险名单不是随便挑出来的。它把资本化率、行业位置、跨期变化和披露模糊度揉在一起，排出了一批更该复核的样本。需注意：风险分是混合信号，名单中存在资本化率为 0% 仍被标异常的样本；这类记录通常由模糊度或跨期变化驱动，不代表激进资本化，必须回原文区分。",
            "",
            "### 结论四：问询函验证说明模型还处在 MVP 阶段",
            "",
            f"在已评估样本中，模型得到 `{_fmt(matrix.get('tp'))}` 个 TP、`{_fmt(matrix.get('fp'))}` 个 FP、`{_fmt(matrix.get('tn'))}` 个 TN、`{_fmt(matrix.get('fn'))}` 个 FN。precision 为 `{_fmt(metrics.get('precision'))}`，recall 为 `{_fmt(metrics.get('recall'))}`。这个结果不能用来吹预测准确率。它反而提醒我们：财务异常信号和交易所问询函不是一一对应的。交易所可能问收入、商誉、资金占用、并购，研发资本化只是其中一类。后面要做全文语义标签，才好判断研发资本化风险到底有没有触发监管关注。",
            "",
            ...
            "### 9. 最终风险分公式",
            "",
            f"当前配置里，行业偏离、跨期变化、条件模糊度的权重分别是 `{_fmt(industry_weight)}`、`{_fmt(change_weight)}`、`{_fmt(fuzziness_weight)}`。配置文件里把恒等式一致性作为最终乘数（`identity_multiplier={_fmt(identity_multiplier)}`），而不是加权项。这样做更符合这个阶段的数据状况：金额关系不稳时，先降低置信度，不把它直接解释成公司风险。",
            "",
            ...
            "### 10. 异常判定模型",
            "",
            f"风险排序不是另训一个模型，而是按风险分排序取前 k 名作为重点复核样本。当前配置 `anomaly_percentile={_fmt(anomaly_percentile)}`，意思是在有风险分的记录里取前 `{_fmt_pct((1 - anomaly_percentile) * 100)}` 左右。异常数 = `ceil(可评分记录数 * (1 - anomaly_percentile))`；当前可评分记录 `{_fmt(scored_count)}` 条，所以取前 `{len(anomalies)}` 条。换算到 150 条全样本，占比才是 `{_fmt_pct(anomaly_share)}`。",
            "",
            "```text",
            ...
            "```",
            "",
            f"当前快照得到 TP=`{_fmt(matrix.get('tp'))}`、FP=`{_fmt(matrix.get('fp'))}`、TN=`{_fmt(matrix.get('tn'))}`、FN=`{_fmt(matrix.get('fn'))}`。这组结果说明：模型已经能生成可复核的风险名单，但问询函弱标签下的预测效果还不强。特别是 `identity_error` 在当前 validate 5% 阈值和 score 0.5 阈值组合下基本不会贡献异常类型，不能把它当成主要风险来源。",
            "",
            "### 13. 当前模型边界",
            "",
            ...
            "| --- | ---: | --- | --- |",
            f"| 理论样本量 | {_fmt(inquiry_count)} | 研究设计覆盖 50 家公司 × 3 年。 | 这不是单家公司个案，而是 company-year 样本。 |",
            f"| 已进入评分的记录 | {_fmt(evaluated_count)} | 当前全量样本覆盖理论样本的 `{_fmt_pct(extraction_coverage)}`。 | 现在能展示全流程，字段缺失会进入质量说明。 |",
            f"| 可计算资本化率记录 | {len(valid_rate_rows)} | 这些记录同时有研发投入和资本化金额。 | 缺字段会影响财务比率，不能硬算。 |",
            f"| 平均资本化率 | {_fmt(cap_rate_avg)}% | 平均水平被少数高资本化样本拉高。 | 平均数说明整体风险水平，但不能代表典型公司。 |",
            f"| 中位数资本化率 | {_fmt(cap_rate_median)}% | 典型样本资本化率较低，分布右偏。 | 多数公司年份并不激进，风险集中在尾部。 |",
            f"| 行业平均资本化率 | {industry_text} | 行业来自 `configs/crawl.yaml` 显式 `industry` 字段，非年报正文识别。 | 所以模型要加入行业偏离度；调整股票池时需同步维护 industry 字段。 |",
            f"| 年度平均资本化率 | {year_text} | 当前快照中 2022 年较高，2023 年较低。 | 年度差异可能来自经济周期、行业样本和 extract 完整度，需要后续复核。 |",
            f"| 异常样本数 | {len(anomalies)} | 在 `{_fmt(scored_count)}` 条可评分记录里取前 20% 左右，再换算到 150 全样本。 | 这 {len(anomalies)} 条就是课堂展示时重点解释的风险清单。 |",
            f"| 异常组 vs 非异常组资本化率 | {_fmt(anomaly_rate_avg)}% vs {_fmt(non_anomaly_rate_avg)}% | 异常组整体资本化率更高，但异常名单是混合信号。 | 个别 0% 资本化率样本可能由模糊度或跨期变化驱动，需回原文复核。 |",
            f"| 异常组 vs 非异常组风险分 | {_fmt(anomaly_score_avg)} vs {_fmt(non_anomaly_score_avg)} | 两组风险分拉开，说明排序机制有效。 | 风险分可以用于排序和抽样复核，不是最终违规判定。 |",
            f"| 有问询候选样本 | {inquiry_with_candidates} | 这些 company-year 在窗口期内找到问询、回复或监管函候选。 | 问询 discovery 已经跑出东西，但还不是人工全文标签。 |",
            f"| 研发资本化相关问询样本 | {len(related_inquiries)} | 关键词命中研发、费用化、无形资产等主题。 | 当前问询标签是 MVP，可用于闭环演示，但需人工复核。 |",
            f"| TP / FP / TN / FN | {_fmt(matrix.get('tp'))} / {_fmt(matrix.get('fp'))} / {_fmt(matrix.get('tn'))} / {_fmt(matrix.get('fn'))} | 模型命中 {_fmt(matrix.get('tp'))} 个相关问询，误报 {_fmt(matrix.get('fp'))} 个，漏报 {_fmt(matrix.get('fn'))} 个。 | 闭环已经跑通，但预测效果还弱。 |",
            f"| Precision / Recall / F1 | {_fmt(metrics.get('precision'))} / {_fmt(metrics.get('recall'))} / {_fmt(metrics.get('f1'))} | 当前风险分和问询标签的一致性较低。 | 结论应写成“方法可行、结果待增强”，不能写成“监管预测准确”。 |",
            f"| Top-K hit rate | {_fmt(metrics.get('top_k_hit_rate'))} | 最高风险样本暂未命中相关问询。 | 最高财务风险不一定立刻对应监管问询，也可能是标签和候选发现还不完整。 |",
            "",
            "### 数据与评分概览",
            "",
            f"- evaluated_records: `{_fmt(evaluated_count)}`",
            f"- scored_records: `{_fmt(scored_count)}`",
            f"- valid_capitalization_rate_records: `{len(valid_rate_rows)}`",
            f"- valid_score_records: `{len(valid_score_rows)}`",
            f"- unscored_records: `{_fmt(unscored_count)}`",
            f"- anomaly_records: `{len(anomalies)}`",
            f"- average_aggressiveness_score: `{_fmt(average_score)}`",
            f"- average_capitalization_rate: `{_fmt(cap_rate_avg)}%`",
            f"- median_capitalization_rate: `{_fmt(cap_rate_median)}%`",
            "",
            "### 问询闭环可行性测试结果",
            "",
            "| TP | FP | TN | FN |",
            "| ---: | ---: | ---: | ---: |",
            (
                f"| {_fmt(matrix.get('tp'))} | {_fmt(matrix.get('fp'))} | "
                f"{_fmt(matrix.get('tn'))} | {_fmt(matrix.get('fn'))} |"
            ),
            "",
            ...
            (
                ...
                f"{_fmt(metrics.get('f1'))} | {_fmt(metrics.get('top_k_hit_rate'))} |"
            ),
            "",
            ...
            "",
            f"为了说明风险评分到底有没有用，而不是只报一个 precision/recall，这里加了最简 baseline 对比。当前正样本只有 `{baselines.get('positive_count', 0)}` 条（全样本 `{baselines.get('evaluated_count', 0)}` 条），统计上接近空，所以三种方法的 precision/recall 都很低——这恰恰是要诚实呈现的：**在当前样本上没有可用的监管预测信号，而不是规则模型“验证成功”**。",
            "",
            "| 方法 | 预测正例数 | TP | FP | FN | precision | recall |",
            "| --- | ---: | ---: | ---: | ---: | ---: | ---: |",
            _baseline_row("规则评分（本项目）", baselines.get("rule_scoring")),
            _baseline_row("按资本化率排序 top20%", baselines.get("cap_rate_top20pct")),
            _baseline_row("全标正（恒判异常）", baselines.get("all_positive")),
            "",
            ...
            "### TN 水分说明",
            "",
            f"闭环矩阵里 TN=`{_fmt(matrix.get('tn'))}`，但其中 `{tn_water.get('tn_unscored_defaulted_non_anomaly', 0)}` 条是因关键字段缺失、没有风险分、被默认归为非异常的记录——它们并没有真正经过模型判定。也就是说，TN 在统计上偏乐观。真要算“模型判过的非异常”，应当扣除这部分。这个口径在 `outputs/loop_evaluation.json` 的 `tn_water` 字段里保留，方便答辩时被问到能直接回答。",
            "",
            "### 图 3：风险评分到问询闭环",
            "",
            ...
            "    R5 --> SCORE_NODE",
            "",
            f"    SCORE_NODE --> ANOMALY_NODE[\"异常名单<br/>{len(anomalies)} 条高风险样本\"]",
            f"    INQUIRY_NODE[\"问询函标签<br/>{len(related_inquiries)} 条研发相关问询\"] --> MATRIX_NODE[\"闭环矩阵<br/>TP={_fmt(matrix.get('tp'))} FP={_fmt(matrix.get('fp'))}<br/>TN={_fmt(matrix.get('tn'))} FN={_fmt(matrix.get('fn'))}\"]",
            "    ANOMALY_NODE --> MATRIX_NODE",
            f"    MATRIX_NODE --> RESULT_NODE[\"阶段性结果<br/>Precision={_fmt(metrics.get('precision'))}<br/>Recall={_fmt(metrics.get('recall'))}\"]",
        ...
        for row in top_k:
            lines.append(
                "| "
                f"{row.get('annual_doc_id', '')} | "
                f"{row.get('company_name', '')} | "
                f"{_fmt(row.get('year'))} | "
                f"{_fmt(row.get('capitalization_rate'))} | "
                f"{_fmt(row.get('aggressiveness_score'))} | "
                f"{_fmt(row.get('inquiry_actually_received'))} | "
                f"{row.get('prediction_result', '')} |"
            )
