...
> ℹ️ **关于早期原型存档**
>
> [`../temp-队友做的东西原件/`](../temp-队友做的东西原件/) 存放的是团队 Week 11–14 的早期原型产物，作为历史参考保留。`project2/` 是在确定主题方向后独立重构的挑战版本。
>
> - **以当前代码为准**：所有核心实现、字段口径、评分逻辑、验证方式均以老师讲义和当前代码为准；早期原型仅供理解项目演进脉络。
> - **接口需独立验证**：外部 API 端点、参数、字段含义和研究结论，均以官方文档、当前代码测试或真实运行日志为准。
> - **开源参考登记**：CNINFO 相关外部仓库作为线索或工程参考，来源、license 和采用边界记录在 [`docs/cninfo_external_references.md`](docs/cninfo_external_references.md)。
...
本项目主题方向由团队共同确定，代码实现经历了渐进式重构：先逐周审读 Week 11–14 的早期原型，理解思路并梳理可改进之处；再在此基础上逐步替换、补全，保留可用设计；最终叠加挑战档要求的评分模型与问询闭环可行性测试，形成当前版本。

早期原型存档于 [`../temp-队友做的东西原件/`](../temp-队友做的东西原件/)，作为历史参考保留；当前版本的实现与结论均以老师讲义和当前代码为准。
...
| 2026-06-13 | 文档整理：统一早期原型存档说明、修正数据覆盖进度、精简故障排查章节、重命名评估模板、整理 prompts 目录、新增 docs/README.md 与 AGENTS.md |
